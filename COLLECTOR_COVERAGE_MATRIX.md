...
| `scripts/windows/host/Invoke-WindowsSecurityAudit.ps1` | Auto | `Host` / `All` | `host/windows-security-audit.json` | Yes | Local host baseline and configuration evidence. Built-in Administrators membership uses SID-first, host-role-aware discovery and preserves unavailable membership as Unknown. |
...
| `scripts/windows/common/Get-WindowsBuiltinGroupInventory.ps1` | Auto | Dot-sourced by local Administrators and RDP collectors | In-memory SID-based group and member objects | Indirect | Shared read-only helper. Uses Active Directory `BUILTIN` groups on domain controllers and LocalAccounts/WinNT on member hosts; it is not a standalone entry point. |
