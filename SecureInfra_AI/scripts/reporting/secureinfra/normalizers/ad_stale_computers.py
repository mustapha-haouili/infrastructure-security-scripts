...
def build_evidence(row: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "never_logged_on": optional_bool(row.get("NeverLoggedOn")),
        "last_logon_evidence": str(row.get("LastLogonEvidence") or ""),
        "last_logon_date_utc": str(row.get("LastLogonDateUtc") or ""),
        "last_logon_timestamp_utc": str(row.get("LastLogonTimestampUtc") or ""),
        ...
        "can_delete_now": optional_bool(row.get("CanDeleteNow")),
        "potential_cleanup_candidate": optional_bool(row.get("PotentialCleanupCandidate")),
        "is_domain_controller": optional_bool(row.get("IsDomainController")),
        "is_server_os": optional_bool(row.get("IsServerOS")),
