...
| 9 | Windows | RDP enabled on many devices | RDP exposure audit with allowed users | Windows | High | Implemented: local host audit | `scripts/windows/host/` exposed through server/workstation scopes | `Get-WindowsRDPExposureAudit.ps1` |
| 10 | Windows | WinRM open without need | WinRM listener and security audit | Windows | Medium | Partial: local listener visibility via network scope | `scripts/windows/network/` and `scripts/windows/server/` | `Get-WindowsNetworkExposureAudit.ps1`; future dedicated WinRM audit |
...
| 12 | Windows | Defender not updated | Microsoft Defender status and signature audit | Windows | High | Partial: workstation inventory implemented | `scripts/windows/workstation/` and `scripts/windows/server/` | `Get-WindowsWorkstationSecurityInventory.ps1`; future server AV expansion |
| 13 | Windows | Firewall profiles disabled | Audit Domain, Private, and Public firewall state | Windows | High | Implemented: network and workstation inventory | `scripts/windows/network/` and `scripts/windows/workstation/` | `Get-WindowsNetworkExposureAudit.ps1`; `Get-WindowsWorkstationSecurityInventory.ps1` |
...
| 16 | Windows | Local admins undocumented | Inventory local administrators per host | Windows | High | Implemented: local host audit | `scripts/windows/host/` exposed through server/workstation scopes | `Get-WindowsLocalAdminInventory.ps1` |
| 17 | Windows | Services with dangerous permissions | Detect unquoted service paths and weak service permissions | Windows | High | Partial: server service account and unquoted path audit implemented | `scripts/windows/server/` and `scripts/windows/workstation/` | `Get-WindowsServerSecurityInventory.ps1`; future ACL depth |
| 18 | Windows | Risky scheduled tasks | Audit scheduled tasks running with high privileges | Windows | Medium | Partial: server scheduled task run-level audit implemented | `scripts/windows/server/` and `scripts/windows/workstation/` | `Get-WindowsServerSecurityInventory.ps1`; future workstation task depth |
